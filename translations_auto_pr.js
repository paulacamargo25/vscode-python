...
const authUser = process.argv[4];
const authToken = process.argv[5];
const userFullName = process.argv[6];
const userEmail = process.argv[7];
const locRootPath = process.argv[8];

if (!repoOwner || !repoName || !authUser || !authToken || !userFullName || !userEmail || !locRootPath) {
    console.error(
...
function hasBranch() {
    ...
    const output = cp.execSync(`git branch --list ${branchName}`);
...
directories.forEach((languageId) => {
    console.log(locRootPath);
    const sourcePath = `${locRootPath}/${languageId}/${repoName}.xlf`;
    const destinationPath = `../vscode-translations-import/${languageId}/${repoName}.xlf`;
